...
pub unsafe fn vp8dx_bool_decoder_fill(mut br: *mut BoolDecoder) {
    unsafe {
        ...
        let mut value: usize = (*br).value;
        let mut count: i32 = (*br).count;
        let mut shift: i32 = VP8_BD_VALUE_SIZE - CHAR_BIT - (count + CHAR_BIT);
        let mut bytes_left: usize = (*br).user_buffer_end.offset_from(bufptr) as usize;
        let mut bits_left: usize = bytes_left.wrapping_mul(CHAR_BIT as usize);
