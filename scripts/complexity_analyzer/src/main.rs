...
fn clone_libvpx(out_dir: &Path) -> PathBuf {
    ...
    if libvpx_path.exists() {
        println!("Directory {:?} already exists, skipping clone.", libvpx_path);
        return libvpx_path;
    }

    println!("Cloning libvpx into {:?}...", libvpx_path);
    run_cmd(&["git", "clone", "https://chromium.googlesource.com/webm/libvpx", libvpx_path.to_str().unwrap()], out_dir);
    libvpx_path
}
...
fn generate_compile_commands(libvpx_path: &Path) {
    ...
    let num_cores = std::thread::available_parallelism().map(|n| n.get()).unwrap_or(1).to_string();
...
fn run_c2rust(libvpx_path: &Path) {
    ...
    run_cmd(&["c2rust", "transpile", "compile_commands.json", "--emit-build-files"], libvpx_path);
}
...
fn gather_file_stats(src_dir: &Path) -> Vec<FileStats> {
    ...
    for entry in WalkDir::new(src_dir).into_iter().filter_map(|e| e.ok()) {
        ...
        if path.extension().map_or(false, |ext| ext == "rs") {
            ...
            if path_str.contains("target/") || path.file_name().unwrap() == "build.rs" || path.file_name().unwrap() == "lib.rs" {
                continue;
            }
...
fn add_top_files(report: &mut String, title: &str, stats: &mut [FileStats], key_fn: fn(&FileStats) -> usize) {
...
fn format_markdown_report(src_dir: &Path, mut stats: Vec<FileStats>) -> String {
    ...
    let mut report = format!(
        ...
        total_unsafe, total_blocks, total_fns, total_statics
    );

    add_top_files(&mut report, "## Top 10 Files (by Total Unsafe Keywords)", &mut stats, |s| s.total);
    add_top_files(&mut report, "## Top 10 Files (by Unsafe Blocks)", &mut stats, |s| s.blocks);
    add_top_files(&mut report, "## Top 10 Files (by Unsafe Functions)", &mut stats, |s| s.fns);
    add_top_files(&mut report, "## Top 10 Files (by Global Mutable State)", &mut stats, |s| s.statics);
